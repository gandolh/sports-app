...
// enters and leaves through `src/persistence/`, and nothing under `src/ui/` may
// read one.
//
// It mirrors the eslint rule that keeps `src/domain/` off the clock, and it is the
// reason `nowIso()` lives in `src/session/useSession.ts` rather than being called
// where it is used.
//
